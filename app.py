...
def index():
    if current_user.name:
        print("ok" + str(current_user.id))
        return render_template('Dashboard.html', current_user = current_user, val = True)
    else:
...
def login():
    if current_user:
        return render_template('Dashboard.html', current_user = current_user, msg = "Loged in!!", val = True)
...
def signup():
    if request.method == 'POST':
        ...
        bank_balance = float(rint(10000,100000))
        user = User(name = user_name, password = user_password, account_num = acct_num, bank_balance = bank_balance, no_of_transactions = 0, assest_balance = 0.0, loan_assest_balance = 0.0)
        print(user_name, user_password, acct_num,bank_balance )
        db.session.add(user)
        db.session.commit()
        load_user(user.id)
        login_user(user)
...
def maketnx():
    if request.method == 'POST':
        ...
        print("Tnx check1")
        print(str(tnx_from), str(current_user.account_num))
        
        if str(tnx_from) == str(current_user.account_num):
            ...
            tnx_id = ref.SHA256(tnx_to + tnx_from + str(tnx_amt) + tnx_type + tnx_type_to)
            
            tnx_info_to = UserTnx(tnx_to = user_to.account_num, tnx_from = current_user.account_num, tnx_amt = tnx_amt, tnx_type = tnx_type, tnx_id = tnx_id ,user_id = current_user.id )
            
            # tnx_info = UserTnx(tnx_to = user_to.name ,tnx_from = tnx_from,tnx_amt = tnx_amt,tnx_type = tnx_type, tnx_id = tnx_id, user_id = current_user.id)
            # tnx_info_to = UserTnx(tnx_to = current_user.name, tnx_from = tnx_from, tnx_amt = tnx_amt, tnx_type = tnx_type_to, tnx_id = tnx_id, user_id = user_to.id)
            
            db.session.add(tnx_info_to)
            db.session.commit()
            print('Done!')
            
            return render_template('Dashboard.html', msg = "Tnx successfully ", val = True)
    print("Error!")
    return render_template('Dashboard.html', msg = "An error Happended")
...
def applylaon():
    if request.method == 'POST':
        ...
        db.session.commit()
        print("yay!")
        return render_template('Dashboard.html', msg = "Loan applied!!", val = True)
    print("Not Yay!")
    return render_template('Dashboard.html')



@app.route('/digitalAssestinfo', methods = ['GET', 'POST'])
@login_required
def digitalAssestinfo():
    if request.method == 'POST':
        ...
        current_user.assest_balance += float(estimated_amount)
        
        db.session.add(migitalmortgage)
        db.session.commit()
        print("Yay!")
        return render_template('Dashboard.html', msg = "Uploaded Successfully", val = True)
...
def api_info_user():
    if request.method == 'GET':
        return current_user.account_num
